...
Fraction 라이브러리: 분수 ADT 구현 (분수 사칙연산, 기약분수 변환, 문자열 ↔ 분수 변환)
Math 라이브러리: 두 정수의 최대공약수(GCD)와 최소공배수(LCM) 계산
Rectangle 프로그램: 명령줄 인자를 이용해 직사각형의 가로, 세로를 입력받아 둘레와 넓이를 계산
...
분수 구조체(Fraction): int numerator, int denominator
사칙연산: add, subtract, multiply, divide
기약분수 변환: simplify
문자열 ↔ Fraction 변환: "A/B" → Fraction (parseFraction), Fraction → "A/B" 또는 정수 출력 (printFraction)
예외 처리: 분모가 0일 경우 오류 메시지 출력 및 프로그램 종료
